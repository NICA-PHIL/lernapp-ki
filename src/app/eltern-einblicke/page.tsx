...
export default function ElternEinblicke() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'radial-gradient(ellipse 80% 50% at 50% -10%, #F0F4FF 0%, #FAFBFF 60%)', fontFamily: "'Inter', -apple-system, sans-serif", paddingBottom: '60px' }}>
      ...
      {/* ── Kompakter Header ── */}
      <div style={{ background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(20px)', padding: '16px clamp(20px,4vw,40px)', display: 'flex', alignItems: 'center', gap: '16px', borderBottom: '1px solid #EEF1F6', position: 'sticky', top: 0, zIndex: 20 }}>
        <button onClick={() => router.push('/dashboard')} style={{ background: '#F3F6FC', border: 'none', borderRadius: '12px', padding: '10px 16px', fontSize: 'clamp(13px,1.1vw,15px)', fontWeight: '700', cursor: 'pointer', color: '#374151' }}>← Zurück</button>
        <div>
          <div style={{ fontWeight: '800', fontSize: 'clamp(16px,1.6vw,19px)', color: '#1A1F36', letterSpacing: '-0.01em' }}>👁️ Einblicke in {childName}s Welt</div>
          <div style={{ fontSize: 'clamp(11.5px,1vw,13px)', color: '#8B92A5' }}>Was Ihr Kind uns über sich selbst erzählt hat</div>
        </div>
      </div>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: 'clamp(20px,3vw,40px)' }}>
        ...
        {/* ── Profil-Kopf ── */}
        <div style={{ background: 'white', borderRadius: '24px', padding: 'clamp(20px,2.2vw,28px)', marginBottom: '20px', boxShadow: '0 8px 28px rgba(79,124,255,0.08)', display: 'flex', alignItems: 'center', gap: '20px', border: '1px solid #EEF1F6' }}>
          {childAvatar ? (
            <img src={childAvatar} style={{ width: 'clamp(56px,5vw,72px)', height: 'clamp(56px,5vw,72px)', borderRadius: '50%', objectFit: 'cover', border: '3px solid #EAF0FF', boxShadow: '0 6px 20px rgba(79,124,255,0.2)' }} />
          ) : (
            <div style={{ width: 'clamp(56px,5vw,72px)', height: 'clamp(56px,5vw,72px)', borderRadius: '50%', background: '#EAF0FF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'clamp(24px,2.5vw,32px)' }}>🧒</div>
          )}
          <div>
            <div style={{ fontWeight: '800', fontSize: 'clamp(19px,2vw,24px)', color: '#1A1F36', letterSpacing: '-0.01em' }}>{childName}</div>
            <div style={{ fontSize: 'clamp(12.5px,1.1vw,14px)', color: '#8B92A5', marginTop: '2px' }}>
              Bevorzugtes Feedback: <strong style={{ color: '#4F7CFF' }}>{belohnungsstil ? STIL_LABELS[belohnungsstil] : 'noch nicht gewählt'}</strong>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(340px, 1fr))', gap: '16px', marginBottom: '16px' }}>
          <div style={{ background: 'white', borderRadius: '22px', padding: 'clamp(18px,2vw,24px)', boxShadow: '0 4px 20px rgba(79,124,255,0.06)', border: '1px solid #EEF1F6' }}>
            <div style={{ fontSize: 'clamp(14px,1.3vw,16px)', fontWeight: '800', color: '#1A1F36', marginBottom: '4px' }}>🌟 Was {childName} wirklich mag</div>
            <p style={{ fontSize: 'clamp(11.5px,1vw,13px)', color: '#9BA3B4', margin: '0 0 14px' }}>Direkt von Ihrem Kind ausgewählt</p>
            {interessen.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {interessen.map(id => { const info = INTERESSEN_LABELS[id]; if (!info) return null; return (
                  <span key={id} style={{ fontSize: 'clamp(12px,1.05vw,13.5px)', fontWeight: '700', padding: '7px 14px', borderRadius: '100px', background: '#FFE8F1', color: '#FF7CB0' }}>{info.icon} {info.label}</span>
                )})}
              </div>
          ...
          <div style={{ background: 'white', borderRadius: '22px', padding: 'clamp(18px,2vw,24px)', boxShadow: '0 4px 20px rgba(79,124,255,0.06)', border: '1px solid #EEF1F6' }}>
            <div style={{ fontSize: 'clamp(14px,1.3vw,16px)', fontWeight: '800', color: '#1A1F36', marginBottom: '4px' }}>🎯 Lernwünsche</div>
            <p style={{ fontSize: 'clamp(11.5px,1vw,13px)', color: '#9BA3B4', margin: '0 0 14px' }}>Fließt direkt in Nica & Phils Umgang ein</p>
            {wuensche.length > 0 ? (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {wuensche.map(id => { const info = WUNSCH_LABELS[id]; if (!info) return null; return (
                  <span key={id} style={{ fontSize: 'clamp(12px,1.05vw,13.5px)', fontWeight: '700', padding: '7px 14px', borderRadius: '100px', background: '#EAF0FF', color: '#4F7CFF' }}>{info.icon} {info.label}</span>
                )})}
              </div>
        ...
        <div style={{ background: 'white', borderRadius: '22px', padding: 'clamp(18px,2vw,24px)', marginBottom: '16px', boxShadow: '0 4px 20px rgba(79,124,255,0.06)', border: '1px solid #EEF1F6' }}>
          <div style={{ fontSize: 'clamp(14px,1.3vw,16px)', fontWeight: '800', color: '#1A1F36', marginBottom: '16px' }}>🎸 Talente-Fortschritt</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '14px' }}>
            {[{icon:'🎸',label:'Gitarre',val:skillsProgress.gitarre,farbe:'#FF8C42',bg:'#FFF0E6'},{icon:'🎹',label:'Piano/Gesang',val:skillsProgress.musik,farbe:'#8A5CFF',bg:'#F2EBFF'},{icon:'♟️',label:'Schach',val:skillsProgress.schach,farbe:'#4F7CFF',bg:'#EAF0FF'}].map(t => (
              <div key={t.label} style={{ textAlign: 'center', padding: 'clamp(14px,1.5vw,18px)', background: t.bg, borderRadius: '18px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                <span style={{ fontSize: 'clamp(20px,2vw,24px)' }}>{t.icon}</span>
                <RingProgress value={t.val} max={10} farbe={t.farbe} />
                <div style={{ fontSize: 'clamp(11px,1vw,12.5px)', fontWeight: '700', color: '#6B7280' }}>{t.label}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: 'white', borderRadius: '22px', padding: 'clamp(18px,2vw,24px)', marginBottom: '20px', boxShadow: '0 4px 20px rgba(79,124,255,0.06)', border: '1px solid #EEF1F6' }}>
          <div style={{ fontSize: 'clamp(14px,1.3vw,16px)', fontWeight: '800', color: '#1A1F36', marginBottom: '14px' }}>✅ Vorbereitung neues Schuljahr</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {checkPhilipp > 0 && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  <span style={{ fontSize: 'clamp(12.5px,1.1vw,14px)', color: '#374151', fontWeight: '600' }}>Philipp</span>
                  <span style={{ fontSize: 'clamp(12.5px,1.1vw,14px)', color: '#4F7CFF', fontWeight: '800' }}>{checkPhilipp}/6</span>
                </div>
                <div style={{ height: '8px', background: '#EEF1F6', borderRadius: '100px', overflow: 'hidden' }}><div style={{ height: '100%', width: `${checkPhilipp/6*100}%`, background: '#4F7CFF', borderRadius: '100px' }} /></div>
              </div>
            )}
            {checkNicole > 0 && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                  ...
                  <span style={{ fontSize: 'clamp(12.5px,1.1vw,14px)', color: '#FF7CB0', fontWeight: '800' }}>{checkNicole}/6</span>
                </div>
                <div style={{ height: '8px', background: '#EEF1F6', borderRadius: '100px', overflow: 'hidden' }}><div style={{ height: '100%', width: `${checkNicole/6*100}%`, background: '#FF7CB0', borderRadius: '100px' }} /></div>
              </div>
            )}
        ...
        <div style={{ background: '#EAF0FF', borderRadius: '18px', padding: 'clamp(14px,1.5vw,18px) clamp(18px,2vw,22px)', display: 'flex', gap: '12px' }}>
          <span style={{ fontSize: 'clamp(16px,1.5vw,18px)' }}>ℹ️</span>
          <p style={{ fontSize: 'clamp(11.5px,1vw,13px)', color: '#4F7CFF', margin: 0, lineHeight: '1.6' }}>
