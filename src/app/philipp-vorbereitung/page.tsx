...
const FAECHER: Fach[] = [
  {
    id: 'deutsch', name: 'Deutsch', icon: '✍️', neu: false, farbe: '#FF7CB0', avatar: 'nica',
  ...
  {
    id: 'mathe', name: 'Mathematik', icon: '🔢', neu: false, farbe: '#4F7CFF', avatar: 'phil',
  ...
  {
    id: 'englisch', name: 'Englisch', icon: '🇬🇧', neu: true, farbe: '#37C978', avatar: 'nica',
  ...
  {
    id: 'gewi', name: 'GEWI (Gesellschaftswissenschaften)', icon: '🌍', neu: true, farbe: '#8A5CFF', avatar: 'phil',
  ...
  {
    id: 'nawi', name: 'NAWI (Naturwissenschaften)', icon: '🔬', neu: true, farbe: '#00C9A7', avatar: 'phil',
  ...
  {
    id: 'kunst', name: 'Kunst & Musik', icon: '🎨', neu: false, farbe: '#FF8C42', avatar: 'nica',
...
export default function PhilippVorbereitung() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F0FAFF', fontFamily: 'system-ui, sans-serif', paddingBottom: '60px' }}>
      ...
      {/* Hero */}
      <div style={{ background: 'linear-gradient(135deg, #4F7CFF, #00C9A7, #37C978)', padding: '32px 24px 70px', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ maxWidth: '680px', margin: '-40px auto 0', padding: '0 20px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '22px', padding: '24px', marginBottom: '24px', boxShadow: '0 8px 28px rgba(79,124,255,0.12)', border: '1px solid #EAF6FF' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '14px' }}>
            <span style={{ fontSize: '24px' }}>🌟</span>
            <h2 style={{ fontSize: '17px', fontWeight: '800', color: '#1A1F36', margin: 0 }}>Die 3. Klasse wird spannend</h2>
          </div>
          ...
          <div style={{ display: 'grid', gap: '10px' }}>
            {[
              ...
              <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'flex-start', background: '#F5FCFF', borderRadius: '14px', padding: '12px 14px' }}>
        ...
        <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <h2 style={{ fontSize: '15px', fontWeight: '800', color: '#1A1F36', margin: 0 }}>Ganz neue Fächer</h2>
          <span style={{ fontSize: '11px', fontWeight: '700', color: '#37C978', background: '#E3FAEE', padding: '3px 10px', borderRadius: '100px' }}>{neueFaecher.length} neu</span>
        </div>
        <p style={{ fontSize: '12px', color: '#9CA3AF', marginBottom: '14px' }}>Hier starten alle bei null — ein fairer, aufregender Neubeginn für Philipp.</p>
        ...
        <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '18px' }}>🔄</span>
          <h2 style={{ fontSize: '15px', fontWeight: '800', color: '#1A1F36', margin: 0 }}>Bekannte Fächer auffrischen</h2>
        </div>
        <p style={{ fontSize: '12px', color: '#9CA3AF', marginBottom: '14px' }}>Diese Fächer kennt Philipp schon — jetzt geht es darum, sicherer zu werden.</p>
        ...
        <div style={{ marginBottom: '10px', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <span style={{ fontSize: '18px' }}>📚</span>
          <h2 style={{ fontSize: '15px', fontWeight: '800', color: '#1A1F36', margin: 0 }}>Philipps Leseliste</h2>
        </div>
        <p style={{ fontSize: '12px', color: '#9CA3AF', marginBottom: '14px' }}>Lesen trainiert nebenbei Rechtschreibung, Wortschatz und Konzentration — hier sind spannende Bücher für den Sommer.</p>

        <div style={{ background: 'white', borderRadius: '18px', padding: '18px', marginBottom: '16px', border: '1px solid #E8ECF4' }}>
          <div style={{ fontSize: '12px', fontWeight: '800', color: '#FF7CB0', marginBottom: '10px' }}>🇩🇪 Deutsch lesen</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {BUECHER_DEUTSCH.map((buch, i) => (
              <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', paddingBottom: i < BUECHER_DEUTSCH.length - 1 ? '10px' : 0, borderBottom: i < BUECHER_DEUTSCH.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                <span style={{ fontSize: '18px' }}>📖</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', fontWeight: '700', color: '#1A1F36' }}>{buch.titel} <span style={{ fontWeight: '400', color: '#9CA3AF' }}>— {buch.autor}</span></div>
                  <div style={{ fontSize: '11.5px', color: '#6B7280', marginTop: '2px' }}>{buch.beschreibung}</div>
                  <span style={{ fontSize: '10px', fontWeight: '700', color: '#FF7CB0', background: '#FFE8F1', padding: '2px 8px', borderRadius: '100px', marginTop: '4px', display: 'inline-block' }}>{buch.level}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ background: 'white', borderRadius: '18px', padding: '18px', marginBottom: '28px', border: '1px solid #E8ECF4' }}>
          <div style={{ fontSize: '12px', fontWeight: '800', color: '#37C978', marginBottom: '10px' }}>🇬🇧 Englisch lesen</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {BUECHER_ENGLISCH.map((buch, i) => (
              <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start', paddingBottom: i < BUECHER_ENGLISCH.length - 1 ? '10px' : 0, borderBottom: i < BUECHER_ENGLISCH.length - 1 ? '1px solid #F3F4F6' : 'none' }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: '13px', fontWeight: '700', color: '#1A1F36' }}>{buch.titel} <span style={{ fontWeight: '400', color: '#9CA3AF' }}>— {buch.autor}</span></div>
                  <div style={{ fontSize: '11.5px', color: '#6B7280', marginTop: '2px' }}>{buch.beschreibung}</div>
                  <span style={{ fontSize: '10px', fontWeight: '700', color: '#37C978', background: '#E3FAEE', padding: '2px 8px', borderRadius: '100px', marginTop: '4px', display: 'inline-block' }}>{buch.level}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Checkliste */}
        <div style={{ background: 'white', borderRadius: '22px', padding: '22px', border: '1px solid #E8ECF4' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '4px' }}>
            <span style={{ fontSize: '20px' }}>✅</span>
            <h2 style={{ fontSize: '16px', fontWeight: '800', color: '#1A1F36', margin: 0 }}>Startklar-Checkliste</h2>
          </div>
          <p style={{ fontSize: '12px', color: '#9CA3AF', margin: '0 0 16px' }}>{checkliste.length}/{checklistePunkte.length} erledigt</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {checklistePunkte.map(p => {
              const done = checkliste.includes(p.id)
              return (
                <button key={p.id} onClick={() => toggleCheck(p.id)}
                  style={{ display: 'flex', alignItems: 'center', gap: '12px', background: done ? '#F0FFF8' : '#FAFAFA', border: `1.5px solid ${done ? '#37C978' : '#E8ECF4'}`, borderRadius: '12px', padding: '12px 14px', cursor: 'pointer', textAlign: 'left', width: '100%' }}>
                  <span style={{ width: '22px', height: '22px', borderRadius: '50%', background: done ? '#37C978' : 'white', border: `2px solid ${done ? '#37C978' : '#D1D5DB'}`, color: 'white', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{done ? '✓' : ''}</span>
...
function FachKarte({ fach, expanded, onToggle, onStart }: { fach: Fach; expanded: boolean; onToggle: () => void; onStart: () => void }) {
  return (
    <div style={{ background: 'white', borderRadius: '18px', border: `2px solid ${expanded ? fach.farbe : '#E8ECF4'}`, overflow: 'hidden', transition: 'all 0.2s' }}>
      <button onClick={onToggle} style={{ width: '100%', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: '14px', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
        <div style={{ width: '42px', height: '42px', borderRadius: '13px', background: `${fach.farbe}18`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', flexShrink: 0 }}>{fach.icon}</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: '800', fontSize: '14px', color: '#1A1F36' }}>{fach.name}</div>
          <div style={{ fontSize: '11px', color: '#9CA3AF' }}>{fach.neu ? 'Neues Fach' : 'Bekanntes Fach'} · mit {fach.avatar === 'nica' ? 'Nica' : 'Phil'}</div>
        </div>
        <span style={{ fontSize: '16px', color: '#9CA3AF', transform: expanded ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>⌄</span>
      </button>
      {expanded && (
        <div style={{ padding: '0 18px 18px' }}>
          <div style={{ background: `${fach.farbe}0D`, borderRadius: '12px', padding: '12px 14px', marginBottom: '12px' }}>
          ...
          <div style={{ marginBottom: '12px' }}>
            <div style={{ fontSize: '11px', fontWeight: '800', color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '8px' }}>Vorbereitung</div>
          ...
          <div style={{ background: '#FFF9E8', borderRadius: '12px', padding: '10px 14px', marginBottom: '14px', display: 'flex', gap: '8px' }}>
            <span style={{ fontSize: '14px' }}>💡</span>
            <p style={{ fontSize: '11.5px', color: '#7A5C00', margin: 0, lineHeight: '1.5' }}>{fach.tipp}</p>
          </div>
          <button onClick={onStart} style={{ width: '100%', padding: '12px', background: fach.farbe, color: 'white', border: 'none', borderRadius: '12px', fontSize: '13px', fontWeight: '800', cursor: 'pointer' }}>
